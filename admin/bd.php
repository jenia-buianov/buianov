...
function time_elapsed_string($datetime,$lang, $full = false) {
    ...
    $string = array(
        'y' => language('yaers',$lang),
        'm' => language('months',$lang),
        'w' => language('weeks',$lang),
        'd' => language('days',$lang),
...
function connect_to_buianov($buianov_name,$buianov_lastname,$buianov_password,$lang,$url)
{
    ...
    curl_setopt($curl, CURLOPT_POSTFIELDS, 'name='.$buianov_name.'&lastname='.$buianov_lastname.'&password='.$buianov_password.'&version='.$version.'&lang='.$lang.'&url='.$url);
	$out = curl_exec($curl);
    curl_close($curl);
